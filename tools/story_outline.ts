...
const paths: { name: string; skip: string[]; picks: string[] }[] = [
  { name: "回敲+聽完+跟著哼+敲三下（全互動）", skip: [], picks: ["回敲三下，不多不少", "聽完，一個字都不打斷", "跟著哼，替她接上下半句", "替那個敲不動的早晨，敲完最後三下"] },
  { name: "回敲+聽完，但出聲安撫（沒接到歌）", skip: [], picks: ["回敲三下，不多不少", "聽完，一個字都不打斷", "出聲安撫她", "沉默地陪著她"] },
  { name: "回敲但關掉收音機", skip: [], picks: ["回敲三下，不多不少", "關掉", "出聲安撫她", "沉默地陪著她"] },
  { name: "開門查看", skip: [], picks: ["開門查看", "聽完，一個字都不打斷", "後退，離牆遠一點", "催她，天亮了，該走了"] },
];
p(`| 代表路徑 | vars | 結局 rank |`);
p(`|---|---|:--:|`);
